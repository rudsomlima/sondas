...
- **Historical recovery search** — `fetchRadiosondyFeatures(year, month, startplace)` hits `export_search.php` to find where a *given past launch's* sonde was recovered. `findRecoveredMatch(features, launchInstant)` picks the GeoJSON point whose timestamp is closest *after* the launch instant, falling back to the closest point *before* it — but **both directions are bounded by `MAX_MATCH_WINDOW_MS` (4h, see `radiosondy.ts`)**. This window was deliberately tightened from an earlier 18h: a wider window let the fallback "steal" the *next* launch's already-correct recovery (two launches 12h apart, e.g. 00Z/12Z, can have their closest-published recovery only ~10h apart when the second flight hasn't landed yet) — confirmed against live radiosondy.info data during development. Real flight duration (ascent+descent to landing) is ~2-2.5h; the window was widened from an initial 3h to 4h after observing flights lasting 3h10-3h30 that fell outside it (see the invariant note near the end of this file).
