...
import { fetchSondeHubRecentFrames, fetchSondeHubArchiveLaunches, SONDEHUB_RECENT_SECONDS } from './sondehub'
...
export async function fetchRecentSondeHubPoints(station: Station): Promise<SondePoint[]> {
  const frames = await fetchSondeHubRecentFrames(station.id, station.lat, station.lon)
  return frames.map(({ serial, frame, association }) => ({
    serial, lat: frame.lat, lon: frame.lon, status: 'UNKNOWN', date: frame.reportDate,
    altitude: frame.alt, sources: ['sondehub'], geographic: association === 'geographic' || undefined,
  }))
}
...
export async function fetchMonthSondePoints(
  ...
  if (opts.recent !== false && monthOverlapsRecentWindow(year, month)) {
    tasks.push(fetchRecentSondeHubPoints(station).then(ps => ps.filter(p => isPointInMonth(p, year, month))))
  }
