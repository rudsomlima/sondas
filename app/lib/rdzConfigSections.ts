...
export const RDZ_CONFIG_SECTIONS: RdzConfigSectionMeta[] = [
  ...
  {
    label: 'Identidade e canais HTTP diretos',
    fields: [
      f('mqtt.prefix', 'Identidade do receptor — usada em toda URL/chave dos canais HTTP diretos', 'string'),
      f('mqtt.report_interval', 'Intervalo de report periódico (ms)', 'int'),
      f('mqtt.siteurl', 'Endereço do app (ex.: https://seuapp.vercel.app) — mesmo campo pra report de bateria/sleep/power, config remota e auto-OTA. Vazio desliga tudo.', 'string'),
    ],
  },
  {
    label: 'Chasemapper',
  ...
  {
    // Renderizada pelo PowerConfigEditor (não campo a campo) — ver
    // FullConfigEditor. Níveis: 0 Pleno, 1 Econômico, 2 Silencioso,
    // 3 Pulsado, 4 Sono profundo (docs/POWER_MODES_GUIDE.md no repo do firmware).
    label: 'Energia',
    fields: [
      f('power.tz', 'Fuso local em minutos (Natal/BRT: -180)', 'int'),
      f('power.w1start', 'Lançamento 1 — início da janela (min locais desde 00:00)', 'int'),
      f('power.w1dur', 'Lançamento 1 — minutos em Pleno (0=desabilitado)', 'int'),
      f('power.w2start', 'Lançamento 2 — início da janela (min locais desde 00:00)', 'int'),
      f('power.w2dur', 'Lançamento 2 — minutos em Pleno (0=desabilitado)', 'int'),
      f('power.wait', 'Espera por lançamento atrasado após cada janela (min)', 'int'),
      f('power.lvl_wait', 'Nível durante a espera (0-3)', 'int'),
      f('power.lvl_idle', 'Nível no resto do dia (0-3, 4=Sono profundo)', 'int'),
      f('power.report_min', 'Silencioso/Pulsado — minutos entre religadas do WiFi', 'int'),
      f('power.pulse_every', 'Pulsado — segundos por ciclo (10-3600)', 'int'),
      f('power.pulse_listen', 'Pulsado — segundos escutando cada frequência (2-120)', 'int'),
      f('power.wakemargin', 'Sono profundo — minutos de folga pra acordar antes da janela', 'int'),
      f('power.landing_wait', 'Minutos sem sinal antes de encerrar o voo', 'int'),
      f('power.vlow', 'Bateria baixa (V): desce 1 nível fora de voo', 'double'),
      f('power.vcrit', 'Bateria crítica (V): desce mais 1 nível', 'double'),
      f('power.vpanic', 'Proteção da célula (V), 0=off: dorme mesmo em voo', 'double'),
      f('power.vcharge', 'Carregando (V), 0=off: acima disso fica em Pleno', 'double'),
      f('power.boost_auto', 'Turbo automático (0=off, 1=on): Pleno quando há sonda no ar perto', 'int'),
      f('power.boost_km', 'Turbo automático — raio (km)', 'int'),
      f('power.boost_min', 'Duração máxima de um turbo (min)', 'int'),
    ],
  },
  {
    label: 'Firmware / auto-OTA',
    fields: [
